...
2. **PR title and description — update the narrative to match the final diff.**

   The current text describes an earlier patch state. The final code no longer contains the advertised `rocm_sdk.__version__` fallback, and hipBLAS no longer hardcodes `HIP_PLATFORM=amd` while preserving `HIP_PLATFORM=nvidia` for `--cuda`; it replaces that interface with `USE_CUDA` and a CUDAToolkit migration. The repository-wide claim is also too broad because the hipRAND and rocRAND package tests still execute `${HIP_HIPCONFIG_EXECUTABLE} --cpp_config`. Revise the title to reflect the hipBLAS, hipBLASLt/TensileLite, shared Tensile, and hipSOLVER scope; describe the two actual change sets; remove stale implementation and test claims; and report the current AMD/CUDA validation status after the blocking failures are resolved.

## Commentary

This change is adjacent to [ROCm/rocm-libraries#11377, `refactor(hipblaslt): remove obsolete preprocessor branches`](https://github.com/ROCm/rocm-libraries/pull/11377) and [ROCm/rocm-libraries#11380, `refactor(tensilelite): reduce redundant feature conditionals`](https://github.com/ROCm/rocm-libraries/pull/11380), but it is not just another dead-branch deletion. PR 11377 removes compile-time-dead alternatives while retaining genuine platform, architecture, and device-pass selectors; PR 11380 removes always-on datatype guards and centralizes the ROCprofiler default. Those PRs classify each condition before removing it, and PR 11377 gives its changed device predicate a focused compile probe. PR 7235 makes a similarly useful move by replacing ambient `HIP_PLATFORM` selection with a CMake option, but then crosses a configuration boundary: package discovery, Python command construction, and target usage requirements all have to agree on that option and its derived values.
